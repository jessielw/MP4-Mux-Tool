...
mp4_root.title("MP4-Mux-Tool v1.15")  # Sets the version of the program
...
def check_audio_tracks_info():
    ...
    def audio_track_choice():  # If audio input has only 1 audio track
        ...
        media_info = MediaInfo.parse(
            audio_input
        )  # Uses pymediainfo to get information for track selection
        for track in media_info.tracks:
            if track.track_type == "Audio":
                audio_track_id_get = str(
                    track.track_id
                )  # Code to save track # into a variable
        acodec_stream = StringVar()  # Makes a new variable
        acodec_stream_choices = {
            "Only One Track": f"#{audio_track_id_get}"
        }  # Temp dictionary with audio track ID
        acodec_stream.set(
            "Only One Track"
        )  # Sets variable to select #1 for command line

    def audio_track_choices(
        *args,
    ):  # If audio input has more then 2 audio tracks, makes a new window to select track
        global acodec_stream, acodec_stream_choices
        ...
        track_frame.grid(
            row=0, column=0, columnspan=5, sticky=E + W, padx=10, pady=(8, 0)
        )
        ...
        for track in media_info.tracks:
            if track.track_type == "Audio":
                if (
                    str(track.format) != "None"
                ):  # Gets format string of tracks (aac, ac3 etc...)
                    audio_format = "|  " + str(track.format) + "  |"
                else:
                    audio_format = ""
                if (
                    str(track.channel_s) != "None"
                ):  # Gets audio channels of input tracks
                    audio_channels = "|  " + "Channels: " + str(track.channel_s) + "  |"
                else:
                    audio_channels = ""
                if (
                    str(track.other_bit_rate) != "None"
                ):  # Gets audio bitrate of input tracks
                    audio_bitrate = (
                        "|  "
                    ...
                    audio_bitrate = ""
                if (
                    str(track.other_language) != "None"
                ):  # Gets audio language of input tracks
                ...
                if str(track.title) != "None":  # Gets audio title of input tracks
                    ...
                    audio_title = ""
                if (
                    str(track.other_sampling_rate) != "None"
                ):  # Gets audio sampling rate of input tracks
                    audio_sampling_rate = (
                        "|  "
                    ...
                    audio_sampling_rate = ""
                if (
                    str(track.other_duration) != "None"
                ):  # Gets audio duration of input tracks
                ...
                if str(track.delay) != "None":  # Gets audio delay of input tracks
                    ...
                    audio_delay = ""
                if (
                    str(track.track_id) != "None"
                ):  # Gets track ID of audio inputs (this is needed for mp4box input)
                    audio_track_id = (
                    ...
                    messagebox.showerror(
                        title="Error!", message="Cannot auto detect track ID"
                    )
                audio_track_info = (
                    audio_format
                    + audio_channels
                ...
                for new_list in [
                    audio_track_info
                ]:  # Take all the pymedia input and adds it into a list
        ...
        for i in range(int(str(total_audio_tracks)[-1])):
            audio_stream_info_output[
                f"Track #{i + 1}:   {result[i]}"
            ] = f"#{audio_track_id_get}"
        # ---------------- Code to take all the information from the newly created list(s) and put it into a dictionary
        ...
        acodec_stream.set(
            next(iter(audio_stream_info_output))
        )  # set the default option
        acodec_stream_menu = OptionMenu(
            track_frame, acodec_stream, *acodec_stream_choices.keys()
        )
        ...
        def close_audio_win():
            mp4_root.attributes(
                "-alpha", 1.0
            )  # Restores mp4root transparency to default
    ...
    if total_audio_tracks is not None and int(total_audio_tracks) == 1:
        ...
        messagebox.showinfo(
            title="Info",
            message=f'"{pathlib.Path(audio_input).name}"\n\nhas 0 audio streams, '
    ...
    if audio_input:
        ...
        if audio_input.endswith(audio_extensions):
            ...
            messagebox.showinfo(
                ...
                "file that should be supported, please let me know.\n\n"
                + 'Unsupported file extension "'
                + str(pathlib.Path(audio_input).suffix)
                + '"',
            )
            audio_delay.set(0)
            audio_language.current(0)
...
audio_input_button = HoverButton(
    audio_tab,
    text="Audio",
    command=audio_input_button_commands,
    foreground="white",
    background="#23272A",
    borderwidth="3",
    activebackground="grey",
...
delete_audio_input_button = HoverButton(
    audio_tab,
    text="X",
    command=clear_audio_input,
    foreground="white",
    background="#23272A",
    borderwidth="3",
    activebackground="grey",
...
delete_audio_input_button.grid(
    row=0, column=3, columnspan=1, padx=10, pady=(10, 5), sticky=E
)
...
subtitle_title_entrybox_label = Label(
    subtitle_tab,
    text="Subtitle Title:",
    anchor=W,
    background="#434547",
...
subtitle_title_entrybox_label.grid(
    row=1, column=1, columnspan=1, padx=10, pady=(0, 0), sticky=W
)
subtitle_title_entrybox = Entry(
    subtitle_tab,
    textvariable=subtitle_title_cmd,
    borderwidth=4,
    background="#CACACA",
    state=DISABLED,
)
subtitle_title_entrybox.grid(
    row=2, column=1, columnspan=3, padx=10, pady=(0, 15), sticky=W + E
)
...
subtitle_language_menu_label = Label(
    subtitle_tab, text="Language:", background="#434547", foreground="white"
)
subtitle_language_menu_label.grid(
    row=1, column=0, columnspan=1, padx=10, pady=(0, 0), sticky=W
)
subtitle_language = ttk.Combobox(
    subtitle_tab,
    values=list(iso_639_2_codes_dictionary.keys()),
    justify="center",
    textvariable=subtitle_language,
)
subtitle_language.grid(
    row=2, column=0, columnspan=1, padx=10, pady=(0, 10), sticky=N + S + W + E
)
...
def subtitle_input_button_commands():
    ...
    subtitle_input = filedialog.askopenfilename(
        initialdir="/",
        title="Select A File",
...
subtitle_input_button = HoverButton(
    subtitle_tab,
    text="Subtitle",
    command=subtitle_input_button_commands,
    foreground="white",
    background="#23272A",
    borderwidth="3",
    activebackground="grey",
...
subtitle_input_button.grid(
    row=0, column=0, columnspan=1, padx=(10, 10), pady=(10, 5), sticky=W + E
)
subtitle_input_button.drop_target_register(DND_FILES)
subtitle_input_button.dnd_bind("<<Drop>>", subtitle_drop_input)

subtitle_input_entry = Entry(
    subtitle_tab, width=39, borderwidth=4, background="#CACACA", state=DISABLED
)
subtitle_input_entry.grid(
    row=0, column=1, columnspan=3, padx=(0, 50), pady=(10, 5), sticky=W + E
)
...
delete_subtitle_input_button = HoverButton(
    subtitle_tab,
    text="X",
    command=clear_subtitle_input,
    foreground="white",
    background="#23272A",
    borderwidth="3",
    activebackground="grey",
...
delete_subtitle_input_button.grid(
    row=0, column=3, columnspan=1, padx=10, pady=(10, 5), sticky=E
)
...
def chapter_input_button_commands():
    ...
    chapter_input = filedialog.askopenfilename(
        initialdir="/", title="Select A File", filetypes=[("OGG", chapter_extensions)]
    )
    if chapter_input:
        chapter_input_entry.configure(state=NORMAL)
        chapter_input_entry.delete(0, END)
...
chapter_input_button = HoverButton(
    chapter_frame,
    text="Chapter",
    command=chapter_input_button_commands,
    foreground="white",
    background="#23272A",
    borderwidth="3",
    activebackground="grey",
...
chapter_input_entry = Entry(
    chapter_frame, width=39, borderwidth=4, background="#CACACA", state=DISABLED
)
chapter_input_entry.grid(
    row=0, column=1, columnspan=3, padx=(0, 50), pady=(10, 0), sticky=W + E
)
...
delete_chapter_input_button = HoverButton(
    chapter_frame,
    text="X",
    command=clear_chapter_input,
    foreground="white",
    background="#23272A",
    borderwidth="3",
    activebackground="grey",
...
delete_chapter_input_button.grid(
    row=0, column=3, columnspan=1, padx=10, pady=(10, 0), sticky=E
)
...
auto_chap_import_checkbox = Checkbutton(
    chapter_frame,
    text="Import chapters from video input",
    variable=auto_chap_import,
    onvalue="on",
    offvalue="off",
    command=save_chap_import_option,
    takefocus=False,
)
auto_chap_import_checkbox.grid(
    row=1, column=0, columnspan=2, rowspan=1, padx=10, pady=(1, 1), sticky=W
)
auto_chap_import_checkbox.configure(
    background="#434547",
    foreground="white",
...
auto_chap_import.set(
    str(config["auto_chapter_import"]["option"])
)  # Set's button status from config.ini
...
def output_button_commands():
    ...
    output_window = filedialog.asksaveasfilename(
...
output_button = HoverButton(
    output_frame,
    text="Output",
    command=output_button_commands,
    foreground="white",
    background="#23272A",
    borderwidth="3",
    activebackground="grey",
...
output_entry = Entry(
    output_frame, width=39, borderwidth=4, background="#CACACA", state=DISABLED
)
output_entry.grid(
    row=0, column=1, columnspan=3, padx=(0, 50), pady=(10, 5), sticky=W + E
)
...
delete_output_button = HoverButton(
    output_frame,
    text="X",
...
def start_job():
    ...
    def error_msg_box():  # Generic error box that shows the error via the 'error_name' variable
        messagebox.showerror(
            title="Error!",
            message="Please input or clear the " + error_name + " input box",
        )
    ...
    try:  # Video is differently checked because it HAS to exist for the program to work, the other
        ...
        video_errors = (
            0  # Set's video_errors to 0 as long as all variables are found correctly
        )
    except (Exception,):
    ...
    try:
        if (
            "audio_input" in globals()
        ):  # If the variable 'audio_input' does exist in globals
            ...
            audio_options = (
                ...
                + audio_delay.get()
                + ":lang="
                + iso_639_2_codes_dictionary[audio_language.get()]
                + ':ID=2"'
            )
        elif (
            "audio_input" not in globals()
        ):  # If the variable 'audio_input' doesn't exist in globals
    ...
    try:
        if (
            "subtitle_input" in globals()
        ):  # If the variable 'subtitle_input' does exist in globals
            total_progress_segments += 1  # Add +1 to total_progress_segments, for final summed count of segments
    ...
    if (
        shell_options.get() == "Default" and total_errors == 0
    ):  # Run block if shell_options = Default and errors = 0

        def close_encode():  # Block of code to close muxing window progress and terminate all sub-processes
            if (
                step_label.cget("text") == "Job Completed"
            ):  # If muxing windows label says 'Job Completed'
                window.destroy()  # Close muxing window only
            else:  # If muxing windows label says anything other than 'Job Completed'
                confirm_exit = messagebox.askyesno(
                    title="Prompt",  # Prompt message box
                    message="Are you sure you want to stop the mux?",
                    parent=window,
                )
        ...
        )  # Set's muxing window title to VideoInput (no path no ext.)
        window.configure(background="#434547")  # Set's muxing window background color
        encode_label = Label(
            window,
            text="- " * 20 + "Progress" + " -" * 20,  # Progress Label
        ...
        encode_window_progress = scrolledtextwidget.ScrolledText(
            window, width=60, height=15, tabs=10, spacing2=3, spacing1=2, spacing3=3
        )
        ...
        step_label = Label(
            window,
            text="Step " + str(0) + " out of " + str(total_progress_segments),
        ...
        auto_close_window_checkbox.grid(
            row=2, column=0, columnspan=1, rowspan=1, padx=10, pady=(10, 0), sticky=W
        )
    ...
    if shell_options.get() == "Default" and total_errors == 0:
        ...
        if (
            config["reset_program_on_start_job"]["option"] == "on"
        ):  # If program is set to reset gui upon successful
        ...
        for (
            ...
            job.stdout
        ):  # Code to put the muxing progress text line by line from stdout into muxing window
            ...
            try:  # Code to break down stdout information
                ...
                if (
                    strip == "00"
                ):  # Each time the code 'strip' says '00' add 1 to var update_number
                    updated_number = updated_number + 1
                    if (
                        updated_number == total_progress_segments
                    ):  # For final step change label to below
                ...
                app_progress_bar["value"] = int(
                    strip
                )  # Code to update the progress bar percentage
            except (Exception,):
                pass
        encode_window_progress.configure(state=NORMAL)
        encode_window_progress.insert(
            END, "Job Completed!!"
        )  # Once job is done insert into scroll box
        encode_window_progress.see(END)
        encode_window_progress.configure(state=DISABLED)
        step_label.configure(
            text="Job Completed"
        )  # Update label to say 'Job Completed' (needed for above code)
    ...
    if (
        shell_options.get() == "Debug" and total_errors == 0
    ):  # Command to muxing process in cmd.exe window
        finalcommand = (
            '"'
            + mp4box
            + video_options
...
def view_command():  # This function is to show the full command line output into a window, the code is the same as
    ...
    try:  # Attempt to update already opened window, this prevents spawning a new command window if it already exsists
        ...
        AttributeError,
        NameError,
    ):  # If no window exists then spawn a new window with all the commands
        ...
        encode_window_progress.grid(
            row=0, column=0, pady=(10, 6), padx=10, sticky=E + W
        )
        encode_window_progress.insert(END, finalcommand)
        encode_window_progress.configure(state=DISABLED)
        ...
        copy_text.grid(
            row=1, column=0, columnspan=1, padx=(20, 10), pady=(15, 2), sticky=W + E
        )
